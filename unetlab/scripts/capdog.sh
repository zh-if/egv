...
for i in $(dc ps | grep Capture- | sed -e 's/.* //') ; do \
	...
	if [ $CON -eq 0 ] ; then \
		...
		# clear tc
		if ( tc qdisc show dev $tapname | grep -q 'netem\|qdisc tbf' ) ; then \
			 tc filter del dev $tapname parent 1:
			 tc filter add dev $tapname  parent 1:  protocol all prio 1 u32 match u32 0 0 flowid 10:1
		else \
			tc filter del dev $tapname parent 1:
		fi
		tc qdisc show dev $tapname | grep -q 'netem\|qdisc tbf' ||  tc qdisc del dev $tapname handle 1: root prio

...
function updateUserToken($db,$username,$pod) {
        ...
        $url = 'http://127.0.0.1/html5/api/tokens';  // Использовать HTTP, а не HTTPS
	...
	$result = (array) json_decode(@file_get_contents($url, false, $context));  // Добавлен @ для подавления ошибок
	if (!$result || !isset($result['authToken'])) {
	    error_log("updateUserToken: Failed to get token for user $username");
	    return false;
	}
